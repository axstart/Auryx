...
When a webhook signing secret is provided by the user (e.g. pasted from a provider dashboard into Replit Secrets), it can carry an invisible trailing space or newline. Comparing `secret.length` against `secret.trim().length` when debugging a signature mismatch will reveal this immediately.
