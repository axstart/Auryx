...
export default function ProtocolFinder() {
  ...
  const copy = dict.protocolFinder;
      ...
      <div className="pt-[calc(var(--site-header-height)+1rem)] pb-8 px-4 sm:px-6 md:px-16 border-b border-border/40">
        <div className="container mx-auto max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-xs uppercase tracking-[0.3em] text-primary mb-3">{copy.eyebrow}</p>
            <h1 className="text-3xl md:text-4xl font-serif leading-snug mb-3">
              {copy.titleBefore}{" "}
              <span className="text-foreground/35">{copy.titleEm}</span>
            </h1>
            <p className="text-foreground/45 text-sm leading-relaxed max-w-lg">{copy.subtitle}</p>
          </motion.div>
        </div>
      </div>
