...
import { getPosts, formatDate, type BlogPost } from "@/data/blog-posts";
...
function PostCard({
  ...
  return (
    <FadeIn delay={index * 0.07}>
      <Link href={langHref(lang, `/blog/${post.slug}`)}>
        <article className="group bg-white border border-[#E0D9CC] rounded-2xl overflow-hidden hover:border-[#C9A844]/40 hover:shadow-lg transition-all duration-300 cursor-pointer h-full flex flex-col">
          ...
          <div className="p-6 flex flex-col flex-1">
            ...
            <p className="text-[12px] text-[#111]/55 leading-relaxed mb-5 flex-1">{post.excerpt}</p>
            <div className="flex items-center justify-between pt-4 border-t border-[#F0EBE1]">
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] font-semibold text-[#111]/70">{post.author}</span>
    ...
    <div className="w-full overflow-x-hidden">
        ...
        <div className="container relative z-10 mx-auto px-6 md:px-14 lg:px-20 pt-[calc(var(--site-header-height)+1rem)] pb-24 md:pt-[calc(var(--site-header-height)+1.5rem)] md:pb-28">
          ...
          >
            ...
            <p className="mt-7 text-[10px] text-white/30 tracking-[0.12em] uppercase">
              {copy.authoredBy.replace("{count}", String(posts.length))}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <div style={{ backgroundColor: "#FAFAF7" }} className="px-6 md:px-14 lg:px-20 py-20">
        <div className="container mx-auto max-w-6xl">
          <FadeIn className="mb-12">
            <p className="text-[10px] uppercase tracking-[0.45em] text-[#B8962E] mb-2 font-medium">{copy.latestEyebrow}</p>
            <div className="flex items-end gap-6">
              <h2 className="font-serif text-3xl text-[#111]">{copy.latestTitle}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {posts.map((post, i) => (
              <PostCard
                ...
                readLabel={copy.read}
                categoryLabel={copy.categories[post.category] ?? post.category}
                lang={lang}
              />
            ))}
          </div>
      ...
      <section className="bg-[#0A0A0A] py-20 px-6 md:px-14 lg:px-20 text-center">
        <div className="container mx-auto max-w-xl">
          <FadeIn>
            <p className="text-[10px] uppercase tracking-[0.45em] text-[#C9A844] mb-5 font-medium">{copy.ctaEyebrow}</p>
            <h2 className="font-serif text-4xl md:text-5xl leading-[1.1] font-light text-white mb-6">
              {copy.ctaTitleBefore}{" "}
              <em className="not-italic text-[#C9A844]">{copy.ctaTitleEm}</em>
            </h2>
            <p className="text-white/50 text-sm leading-relaxed mb-10 max-w-md mx-auto">{copy.ctaBody}</p>
            <Link
              href={langHref(lang, "/protocol-finder")}
